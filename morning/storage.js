 ...
 * 所有键以 "morning:" 为前缀，JSON 序列化。
 * 读取一律返回新对象，绝不返回存储内部的引用。
 */
const Storage = (() => {
const KEY_SETTINGS = 'morning:settings';
const KEY_RECORDS = 'morning:records';
  ...
  return {
    ...
    /**
     * 判定今日推荐模式：昨天00:00至今存在 completed && dayType==='full' 的记录 → 'stretch'，否则 'full'。
     * 旧记录无 dayType → 按 resistanceTimeSec>0 推导。
     */
    getRecommendedMode() {
      const records = this.getRecords();
      const now = new Date();
